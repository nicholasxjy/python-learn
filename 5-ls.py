...
def main():
	...
	print("{0} file{1}, {2} director{3}".format("{0:n}".format(counts[0]) if counts[0] else "no", "s" if counts[0] !=1 else "",
...
def process_options():
	...
	parser.add_option("-m", "--modifide", dest="modified",
		action="store_true",
	...
	for name in filenames:
		...
		if opts.modified:
			try:
				...
				modified = "{0:>19}".format("unknow")
		size = ""
		if opts.sizes:
			try:
				size = "{0:>15n} ".format(os.path.getsize(name))
			except EnvironmentError:
				size = "{0:15n} ".format("unknow")
