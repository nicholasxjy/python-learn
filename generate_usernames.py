...
def main():
    ...
    for filename in sys.argv[1:]:
        for line in open(filename, endcoding='utf-8'):
            line = line.rstrip()
            if line:
...
def print_users(users):
    namewith = 32
    usernamewith = 9
    print("{0:<{nw}}{1:^6}{2:{uw}}".format("Name", "ID", "Username", nw=namewith, uw=usernamewith))
    print("{0:-<{nw}}{0:-<6}{0:-<{uw}}".format("", nw=namewith, uw=usernamewith))
    ...
    for key in sorted(users):
        ...
        print("{0:.<{nw}}({1.id:4}){1.username:{uw}}".format(name, user, nw=namewith, uw=usernamewith))