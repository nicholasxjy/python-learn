...
def xdump(filename, blocksize, opts.encoding, opts.decimal):

    encoding_text = "{0} characters".format(encoding.upper())
    with = (blocksize * 2) + (blocksize // 4)
    if blocksize % 4:
        width += 1
    print("-------- {0}{1}".format("-" * (with-1), "-" * max(len(encoding_text), blocksize)))
    ...
    try:
        ...
        while True:
            ...
            for i in range(len(data), blocksize):
                ...
                line.append("  ")
            line.append("  ")
            line.append(bytes(chars).decode(encoding, "replace").replace("\uFFD", "."))
            print("".join(line))
            block_number += 1
